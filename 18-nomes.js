...
(function(){
  ...
  // "Legenda" = o texto que cada badge/ponto gera. Só entram as que já
  // foram efetivamente "criadas": nas Superiores, isso é copiada pelo
  // menos uma vez (mesmo critério de uso que os outros botões desse
  // bloco); nas Inferiores, é ter sido clicada (já é assim que `filled`
  // funciona -- pontos nunca clicados não geram nada). O número de cada
  // linha é o mesmo já mostrado na tela: 01-11 fixo nas Superiores,
  // posição + numeração inicial nas Inferiores.
  ...
  function exportGeneratedNamesCSV(){
    ...
    STATIC_NAMES.forEach(function(text, idx){
      if (staticCopied[idx]) rows.push({ num: idx + 1, legenda: text });
    });
